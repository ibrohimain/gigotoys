...
import { Send, CheckCircle2, Package, ShoppingBag, Box, Target, TrendingUp, Clock, AlertTriangle, Trophy, BarChart3 } from 'lucide-react';
...
const AgentDashboard: React.FC<AgentDashboardProps> = ({ profile }) => {
  ...
  const personalStats = useMemo(() => {
    ...
    // CRITICAL: Only count APPROVED reports for final results and plan progress
    ...
    return { todaySales, monthSales, planProgress, planBreakdown, approvedCount: approved.length };
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Tasdiqlangan Bugun</p>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Oylik Natija</p>
          <div className="flex items-center gap-2">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 lg:col-span-2 flex items-center justify-between">
          ...
          <div className="flex -space-x-2">
             {bonusTiers.map((tier) => (
               <div key={tier.id} className={`w-10 h-10 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-black shadow-sm ${personalStats.planProgress >= tier.threshold ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400'}`} title={`${tier.threshold}% - ${tier.prize}`}>
                 {tier.threshold}%
               </div>
             ))}
          </div>
      ...
      {activePlan && (
        <div className="bg-indigo-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h3 className="text-xl font-black mb-2 flex items-center gap-2"><Target /> Plan: {activePlan.totalAmount.toLocaleString()} so'm</h3>
            <p className="text-indigo-200 text-xs mb-6 italic">Faqat direktor tasdiqlagan savdolar bu yerda aks etadi.</p>
            
            <div className="relative pt-6 pb-2">
              <div className="h-4 bg-white/20 rounded-full w-full mb-8 overflow-hidden">
                <div 
                  className="h-4 bg-gradient-to-r from-green-400 to-emerald-500 rounded-full transition-all duration-1000" 
                  style={{ width: `${Math.min(personalStats.planProgress, 100)}%` }} 
                />
              </div>
              
              {bonusTiers.map(tier => (
                <div key={tier.id} className="absolute top-0 flex flex-col items-center" style={{ left: `${tier.threshold}%`, transform: 'translateX(-50%)' }}>
                  ...
                  <span className="text-[8px] font-black text-indigo-200">{tier.threshold}%</span>
                </div>
              ))}
            </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-2xl font-black text-gray-900">Yangi Hisobot Yuborish</h3>
          {isDebtOverLimit && (
            <div className="flex items-center gap-2 bg-red-50 text-red-600 px-4 py-2 rounded-xl text-xs font-bold">
              <AlertTriangle className="w-4 h-4" /> Qarz limiti! ({currentDebtPercent.toFixed(1)}%)
            </div>
          )}
        </div>

        {success && (
          <div className="mb-8 p-4 bg-green-50 text-green-700 rounded-2xl flex items-center gap-3 animate-bounce">
            <CheckCircle2 className="w-6 h-6" />
            <p className="font-bold">Hisobot direktorga tasdiqlash uchun yuborildi!</p>
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <ProductCard type="QURT" label="Qurt" icon={Package} color="amber" val={sales.QURT} onChange={handleInputChange} />
            <ProductCard type="OYINCHOQ" label="O'yinchoq" icon={ShoppingBag} color="blue" val={sales.OYINCHOQ} onChange={handleInputChange} />
          ...
          <div className="bg-gray-50 p-6 rounded-3xl flex flex-wrap items-center justify-between gap-6">
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase">Hozirgi hisobot summasi</p>
            ...
            >
              {loading ? <Clock className="animate-spin" /> : 'Direktorga Yuborish'}
            </button>
          </div>
        </form>
      </div>
...
const ProgressMini = ({ type, current, target }: any) => (
  <div className="bg-white/10 p-4 rounded-2xl border border-white/5">
    ...
    <div className="h-1 bg-white/10 rounded-full mt-2">
      <div className="h-1 bg-white rounded-full transition-all duration-500" style={{ width: `${Math.min((current/target)*100 || 0, 100)}%` }} />
    </div>
  </div>
);
...
const ProductCard = ({ type, label, icon: Icon, color, val, onChange }: any) => (
  <div className={`p-6 rounded-3xl bg-white border border-gray-100 hover:border-${color}-400 transition-all space-y-4`}>
    <div className="flex items-center gap-3">
      <div className={`p-2 bg-${color}-50 text-${color}-600 rounded-xl`}><Icon className="w-6 h-6" /></div>
      <span className="font-black text-gray-800">{label}</span>
    </div>
    <div className="space-y-3">
       <input type="number" placeholder="Summa" className="w-full p-3 bg-gray-50 rounded-xl font-bold outline-none" value={val.totalPrice || ''} onChange={(e) => onChange(type, 'totalPrice', e.target.value)} />
       <input type="number" placeholder="Qarz" className="w-full p-3 bg-red-50 rounded-xl font-bold text-red-600 outline-none" value={val.debtAmount || ''} onChange={(e) => onChange(type, 'debtAmount', e.target.value)} />
       <input type="number" placeholder="Soni" className="w-full p-3 bg-gray-50 rounded-xl text-sm outline-none" value={val.quantity || ''} onChange={(e) => onChange(type, 'quantity', e.target.value)} />
    </div>
  </div>
);
